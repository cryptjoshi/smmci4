...
import { getToken } from 'app/actions/userInfof';
  ...
  export default function SumWinlossDetail(props:any) {
  ...
  const createPages = (count: number) => {
    ...
    const columns = [
      columnHelper.accessor('id', { id:'id',header: () => 'ID', cell: (info:any) => parseInt(info.row.id) + 1, }),
      ...
      columnHelper.accessor('GameRoundID', {
        ...
        cell: (info:any) => (
          <Link href={`/admin/sumwinloss/detail/${id}/${info.getValue()}`}>
          <Button >
      ...
      columnHelper.accessor('BetAmount', {
        ...
        footer: ({ table }:any) => {
          const total = table
           ...
           return ( <Text color={ 'blue.500'} fontSize="md" fontWeight="600">{total.toFixed(2)}</Text>)
        },
      }),
      ...
      columnHelper.accessor('PayoutAmount', {
        ...
        footer: ({ table }:any) => {
          ...
          : 'red.500'
            ...
            fontSize="sm" fontWeight="600">{total.toFixed(2)}</Text>)
        },
      }),
      ...
      columnHelper.accessor('TURNOVER', {
        ...
        cell: (info:any) => (
          <Text color={info.getValue()>=0?'blue.300':'gray.200'}  fontSize={{ sm: '10px', lg: '12px' }} fontWeight="600">
        ...
        footer: ({ table }:any) => {
          ...
          fontSize={{ sm: '10px', lg: '12px' }} fontWeight="600">{total.toFixed(2)}</Text>)
        },
      }),
