...
export default function OverallRevenue(props: { [x: string]: any }) {
	...
	useEffect(() => {
		...
		const checkData = async () => {
		  ...
		  } else 
		  {
			...
			let res = await fetch('https://report.tsxbet.net/reports/count/sumonthly', { method: 'POST',
			  headers: {
  ...
  const reLoad = (event:any)=>{
    ...
    // lineChartOptionsOverallRevenue.xaxis.categories = ["1","2","3"]
   }
    ...
    >
      <Flex justify="space-between" px="20px" pt="5px" w="100%">
        ...
        <option value="daily">วันนี้</option>
          ...
          <option value="last_monthly">เดือนที่แล้ว</option>
        </Select>
      </Flex>
