...
use Modules\Addfunds\Models\add_funds_model;


class Addfunds extends BaseController {
	public $tb_users;
	...
	public function __construct(){
		...
		$this->model = new add_funds_model();
		$this->tb_users            = USERS;
	...
	public function index(){
		$data = array(
			"module"        => "add_funds",
			"model"         => $this->model
			//"payment"		=> $this->model->get("select ")
		);
		
		return view('Modules\Addfunds\Views\index', $data);
	}
	...
	public function process(){
		...
		$total_amount = $amount + (((float)$amount*(float)$transaction_fee)/100);

		if (in_array($payment_method, ['coinbase', 'hesabe', 'dotpay', 'shopier', 'perfectmoney'])) {
			$data = array(
	...
	public function omise(){
		$omise = new \Modules\Addfunds\Controllers\Omise();
		return $omise->index();
