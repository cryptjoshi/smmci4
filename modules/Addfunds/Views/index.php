          ...
          <div class="card-body">
            <div class="tab-content">
              ...
              <?php
                if (get_option("is_active_razor_pay")) {
              ?>
              <div id="razor_pay" class="tab-pane fade">
                <form class="form actionForm" action="<?=cn($module."/process")?>" data-redirect="<?=cn($module."/razor_pay_form")?>" method="POST">
                  <div class="row">
                    <div class="col-md-12">
                      <?php 
                        if (get_option("razor_pay_secret_key", '') != "" && get_option("razor_pay_publishable_key", '') != "") {
                      ?>
                      <div class="for-group text-center">
              ...
              <?php
                ...
                if (!empty($payments_method) && is_array($payments_method)) {
                  foreach ($payments_method as $payment) {
                    ...
                    if (payment_method_exists($payment) && get_option('is_active_'.$payment) || $payment == "Omise") {
                     // echo "select * from payments where type='{$payment}'";
                      $result = $model->db->query("select * from payments where type='{$payment}'")->getResultArray();
                     
                      $data = array(
                        'module' => $module,
                        'payment' => (object)$result[0]
                      );
                     
                      echo view('Modules\Addfunds\Views/'.$payment.'/index', $data);
                    }
                  }
                }
              ?>
