    ...
    <div class="col-md-4">
      <div class="card">
        <div class="card-header d-flex align-items-center">
          <h3 class="h4"><i class="fa fa-ticket"></i> <?=lang("app.ticket_no")?><?=$ticket->id?></h3>
        </div>
        <div class="card-body">
          <div class="ticket-details">
            <table class="table">
              <tbody>
                <tr>
                  <td scope="row"><?=lang("app.status")?></td>
                  <td>
                    <?php
                      $button_type = "info";
                    ...
                    <div class="btn-group">
                      ...
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right has-shadow">
                          ...
                          <a href="javascript:void(0)" data-url="<?=cn($module."/ajax_change_status/".$ticket->ids)?>" data-status="closed" class="ajaxChangeStatus dropdown-item"><?=lang("app.submit_as_closed")?></a>
                          <a href="javascript:void(0)" data-url="<?=cn($module."/ajax_change_status/".$ticket->ids)?>" data-status="pending" class="ajaxChangeStatus dropdown-item"><?=lang("app.submit_as_pending")?></a>
                          <a href="javascript:void(0)" data-url="<?=cn($module."/ajax_change_status/".$ticket->ids)?>" data-status="answered" class="ajaxChangeStatus dropdown-item"><?=lang("app.submit_as_answered")?></a>
                          <a href="javascript:void(0)" data-url="<?=cn($module."/ajax_change_status/".$ticket->ids)?>" data-status="unread" class="ajaxChangeStatus dropdown-item"><?=lang("app.mark_as_unread")?></a>
                        </div>
      ...
      <div class="card">
        ...
        <form class="card-body form actionForm m-t-20" action="<?=cn($module."/ajax_update/".$ticket->ids)?>" data-redirect="<?=cn("$module/view/".$ticket->id)?>" method="POST">
          <div class="form-group">
            <label for="userinput8"><?=lang("app.message")?></label>
            <textarea rows="10" class="form-control square plugin_editor" name="message" ></textarea>
          </div>
          <button type="submit" class="btn round btn-info btn-min-width mr-1 mb-1"><?=lang("app.submit")?></button>
        </form>
        <?php }?>
      </div>
    </div>
