...
use Modules\Tickets\Models\tickets_model;
...
class tickets extends BaseController {
	...
	public function __construct(){
		//parent::__construct();
		$this->model = new tickets_model(); //("tickets".'_model', 'model');
		//Config Module
		$this->tb_users      = USERS;
	...
	public function index(){
		...
		$config = array(
			'base_url'           => cn("tickets".$query_string),
		...
		if (get_option("is_clear_ticket", "")) {
			...
			if (!empty($old_tickets)) {
				foreach ($old_tickets as $key => $row) {
					$this->model->db->delete($this->tb_ticket_message, ['ticket_id' => $row->id]);
					$this->model->db->delete($this->tb_tickets, ['id' => $row->id]);
				}
			}
		}
		...
		return view("Modules\Tickets\Views\index", $data);
	}
	...
	public function add(){
		...
		return view('Modules\Tickets\Views\add', $data);
	}	
	...
	public function view($id = ""){
		...
		if (!empty($ticket)) {
			...
			if (get_role('admin') || get_role('subporter')) {
				$this->model->common_update($this->tb_tickets, ['admin_read' => 0], ['ids' => $ticket->ids]);
			}else{
				$this->model->common_update($this->tb_tickets, ['user_read' => 0], ['ids' => $ticket->ids]);
			}
			...
			return view('Modules\Tickets\Views\update', $data);
		}else{
			load_404();
		}
	}
	...
	public function ajax_add(){
		...
		$this->model->common_insert($this->tb_tickets, $data);
		if ($this->model->db->affected_rows() > 0) {

			if (get_option('is_ticket_notice_email_admin', 0) && get_role('user')) {
				$ticket_id = $this->model->insertID();
			...
			ms(array(
				"status"  => "success",
				"message" => lang("app.ticket_created_successfully")
			));
	...
	public function ajax_update($ids){
		...
		if(!empty($check_item)){
			...
			$this->model->common_insert($this->tb_ticket_message, $data);
			if ($this->model->db->affected_rows() > 0) {
				...
				$this->model->common_update($this->tb_tickets, $data_ticket, ["id" => $check_item->id]);
	...
	public function ajax_change_status($ids){
		...
		if(!empty($check_item)){
			...
			$this->model->common_update($this->tb_tickets, $data, ["ids" => $ids]);
			if ($this->model->db->affected_rows() > 0) {
				ms(array(
					"status"  => "success",
	...
	public function search(){
		...
		$config = array(
			'base_url'           => cn("tickets"."/search".$query_string),
	...
	public function ajax_search(){
		...
		return view("Modules\Tickets\Views\ajax_search", $data);
	}
