<?php
namespace Modules\Api_provider\Controllers;
use App\Controllers\BaseController;
use Modules\Api_provider\Models\api_provider_model;
//defined('BASEPATH') OR exit('No direct script access allowed');
//   ini_set('display_errors', '1');
//  ini_set('display_startup_errors', '1');
//  error_reporting(E_ALL);
class api_provider extends BaseController {
	public $tb_users;
	...
	public function __construct(){
		...
		$this->model =  new api_provider_model();
		$this->tb_users       		= USERS;
	...
	public function index(){
		...
		return view('Modules\Api_provider\Views\index', $data);
	}
	...
	public function update($ids = ""){
		...
		return view('Modules\Api_provider\Views\update', $data);
	}
	...
	public function ajax_update($ids = ""){
		...
		if(empty($check_item)){
			...
			$data["changed"] = NOW;
			$data["created"] = NOW;
			$this->model->common_insert($this->tb_api_providers, $data);
		}else{
			$data["changed"] = NOW;
			$this->model->common_update($this->tb_api_providers, $data, array("ids" => $check_item->ids));
		}
		...
		if ($ids != "" ) {
			...
			if (!empty($api)) {
				...
				if (empty($data_connect) || !isset($data_connect->balance)) {
					...
					$this->model->common_update($this->tb_api_providers, $data, ["ids" => $api->ids]);
	...
	public function services(){
		...
		return view('Modules\Api_provider\Views\api/get_services', $data);
	}
	...
	public function sync_services($ids = ""){
		if (!empty($ids)) {
			...
			if (!empty($api)) {
				...
				return view('Modules\Api_provider\Views\api/sync_update', $data);
			}
		}
	}
	...
	public function ajax_sync_services($ids){
		...
		if ($ids != "") {
			...
			if (!empty($api)) {
				...
				return view('Modules\Api_provider\Views\api\result_sync',$data);
	...
	public function auto_sync_services_setting(){
		...
		return view('Modules\Api_provider\Views\api\auto_sync_update',$data);
	...
	private function sync_services_by_api($data_item){
		...
		if (!empty($disabled_services)) {
			foreach ($disabled_services as $key => $disabled_service) {
				$this->model->common_update($this->tb_services, ["status" => 0, "changed" => NOW], ["api_provider_id" => $api->id, "api_service_id" => $disabled_service->service, 'id' => $disabled_service->id]);
			}
		}
		...
		if (!empty($exists_services) && !empty($enable_sync_options)) {
			foreach ($exists_services as $key => $exists_service) {
				...
				/*----------  sync Servie Name  ----------*/
				if (isset($enable_sync_options['service_name']) && $enable_sync_options['service_name']) {
					if (isset($exists_service->desc) && $exists_service->desc != "") {
				...
				$this->model->common_update($this->tb_services, $data_service, ["api_service_id" => $exists_service->service, "api_provider_id" => $api->id]);
			}
		}
		...
		if (!empty($new_services) && $request) {
			...
			foreach ($new_services as $key => $new_service) {
				/*----------  Is exists Services or not ----------*/
				$exists_service = $this->model->get('ids', $this->tb_services, ['api_service_id' => $new_service->service, 'api_provider_id' => $api->id]);
				if ($exists_service) {
					continue;
				}
				...
				if (!empty($check_category)) {
					...
					$this->model->common_insert($this->tb_categories, $data_category);

					if ($this->db->affectedRows() > 0) {
						$cate_id = $this->model->insertID();
						$data_service["cate_id"] = $cate_id;
					}
				}
				if (!empty($data_service)) {
					$this->model->common_insert($this->tb_services, $data_service); 
				}
				++$i;
			}
		...
		$this->model->common_update($this->tb_api_providers, ['changed' => date('Y-m-d H:i:s', strtotime(NOW) + $rand_time)], ['id' => $api->id]);
	...
	public function bulk_services($ids = ""){
		if (!empty($ids)) {
			...
			if (!empty($api)) {
				...
				return view('Modules\Api_provider\Views\api/bulk_update', $data);
			}
		}
	}
				...
				foreach ($data_services as $key => $row) {
					...
					// 	/*----------  check Category and add it  ----------*/
					 	...
					 	if(empty($check_services)){
					 		...
					 		$category_name  = strip_tags(trim($row->category));
							...
							$this->model->common_update($this->tb_services, $data_service, ["api_service_id" => $row->service, "api_provider_id" => $api->id, "ids" => $check_services->ids ]);
					 	}
					 }else{
					 	break;
					 }
	...
	public function ajax_api_provider_services($ids = ""){
		if (!empty($ids)) {
			...
			if (!empty($api)) {
				...
				return view("Modules\Api_provider\Views\api/ajax_get_services", $data);
			}else{
				$message = '<div class="alert alert-icon alert-danger" role="alert"> <i class="fe fe-alert-triangle mr-2" aria-hidden="true"></i> '.lang("app.there_seems_to_be_an_issue_connecting_to_api_provider_please_check_api_key_and_token_again").'</div>';
				echo $message;
			}
	...
	public function ajax_add_api_provider_service(){
		...
		$check_item = $this->model->get("ids", $this->tb_services, ['api_provider_id' => $api_provider_id, 'api_service_id' => $api_service_id]);
		...
		if(empty($check_item)){
			...
			$data["changed"] = NOW;
			$data["created"] = NOW;
			$this->model->common_insert($this->tb_services, $data);
		}else{
			$this->model->common_update($this->tb_services, $data, ["ids" => $check_item->ids]);
		}
	...
	public function cron($type = ""){
		switch ($type) {
			...
			case 'refill':
				$refills = $this->model->get_all_refill();
				if (!empty($refills)) {
					foreach ($refills as $key => $row) {
				 ...
				 if (!empty($api)) {
					...
					if (isset($response->error) && $response->error != "") {
						...
						$this->model->common_update($this->tb_refills, $data, ["id" => $row->id]);
						continue;
					}

					if (!empty($response->status) && $response->status != "") {
						$this->model->common_update($this->tb_refills, ["status" => $response->status, "changed" => NOW], ["id" => $row->id]);
					}
					}else{
					...
					echo "There is no refill at the present.<br>";
				}
				echo "Successfully";
				break;
			case 'order':
				...
				if (!empty($orders)) {
					foreach ($orders as $key => $row) {
						...
						if (!empty($api)) {
							...
							if ($response) {
								// Update Note for admin if Get order issue
								if (isset($response->error) && $response->error != "") {
									echo "OrderID - ". $row->id." :". $response->error."<br>";
									$data = array(
										"status"      => 'error',
									...
									$this->model->common_update($this->tb_orders, $data, ["id" => $row->id]);
									continue;
								}

								if (!empty($response->order) && $response->order != "") {
									$this->model->common_update($this->tb_orders, ["api_order_id" => $response->order, "changed" => NOW], ["id" => $row->id]);
								}
				...
				if (!empty($orders)) {
					foreach ($orders as $key => $row) {
						...
						if (!empty($api)) {
								...
								if (!empty($data)) {
									...
									if ($row->sub_response_posts != 1 && ($response->status == "Refunded" || $response->status == "Canceled" || $response->status == "Partial" )) {
										...
										if (!empty($user)) {
											...
											$this->model->common_update($this->tb_users, ["balance" => $balance], ["id"=> $row->uid]);
										}
									}
									if(strtolower($response->status)=="completed"){
										$data['refill_status'] = "no";
									}else {
										$data['refill_status'] = "";
									}
									if($response->is_refill==null)
									{
										$data['is_refill'] = "yes";
									}
									$this->model->common_update($this->tb_orders, $data, ["id" => $row->id]);
							...
							echo $row->id."<br>";
							echo $row->api_provider_id." API Provider does not exists.<br>";
						}
					}
	...
	private function insert_order_from_dripfeed_subscription($main_order = "", $provider = "", $new_dripfeed_orders = "" ){
		...
		foreach ($new_dripfeed_orders as $key => $order_id) {
			...
			$data_order['formal_charge'] = ($service->original_price * $total_charge) / $user_service_price;
	...
	private function update_fund_to_user($uid, $funds, $type = ""){
		...
		if (!empty($user)) {
			...
			$this->model->common_update($this->tb_users, ["balance" => $balance], ["id"=> $uid]);
		}
	}
