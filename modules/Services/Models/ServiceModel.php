<?php namespace Modules\Services\Models;
use App\Models\ExtendModel;


class ServiceModel  extends ExtendModel{
	public $tb_users;
...
public function get_services_list($user_level=""){
    return  $this->model->get_services_list($user_level);
}

// public function get_services_list_page($user_level = "",$total_rows = false, $status = "", $limit = "", $start = "",$cate_id=""){
...
// 	}
	...
	public function get_active_categories(){
		$data  = array();
		// get categories
		$builder = $this->db->table($this->tb_categories);
		if (get_role("user")) {
			$builder->where("status", "1");
		}
		$builder->select("id, ids, name, data, classname");
		//$this->db->from($this->tb_categories);
