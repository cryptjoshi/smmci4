                    ...
                    <div class="modal-body">
                      ...
                      ?>
                                            <option value="<?=$category->id?>"
                                                <?=(!empty($service->ids) && $category->id == $service->cate_id)? 'selected': ''?>>
                        ...
                        ?>
                                                <option value="<?php echo strip_tags($api_provider->id)?>"
                                                    <?php echo (isset($service->api_provider_id) && $service->api_provider_id == $api_provider->id)? 'selected': ''?>>
                                                    <?php echo strip_tags($api_provider->name)?></option>
                                                <?php }} ?>
                                            </select>
                                        </div>
                                            ...
                                            <div class="dimmer">
                                                ...
                                                <div class="dimmer-content">
                                                    ...
                                                    <input type="hidden" class="form-control square"
                                                        name="api_service_id"
                                                        value="<?php echo (!empty($service->api_service_id))? $service->api_service_id : '' ?>">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row api-service-details d-none">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control square" name="original_price"
                                                        value="<?php echo (!empty($service->original_price))? $service->original_price : '' ?>"
                                                        disabled>
                                                    <input type="hidden" class="form-control square"
                                                        name="original_price"
                                                        value="<?php echo (!empty($service->original_price))? $service->original_price : '' ?>">
                                                    ...
                                                    <input type="hidden" class="form-control square"
                                                        ...
                                                        value="<?php echo (!empty($service->type))? $service->type : 'default' ?>">

                                                    <input type="hidden" class="form-control square"
                                                        name="api_service_dripfeed"
                                                        value="<?php echo (!empty($service->dripfeed))? $service->dripfeed : '' ?>">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row api-service-details d-none">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <label class="custom-switch">Refill </label>
                                                        <input type="checkbox" name="item_is_refill"
                                                            class="custom-switch-input ajaxToggleItemStatus"
                                                            <?php if(!empty($service->is_refill) && $service->is_refill == 1) echo 'checked'; ?>>
                                ...
                                <!-- Manual Mode -->
                                <div
                                    class="col-md-12 service-manual-type <?php echo (!isset($service->add_type) || (isset($service->add_type) && $service->add_type == 'manual')) ? '' : 'd-none'?>">
                                    <fieldset class="form-fieldset">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="service_type"
                                                        class="form-control square ajaxChangeServiceType">
                                                        <?php
                              $service_type_array = all_services_type();
                              foreach ($service_type_array as $type => $service_type) {
                            ?>
                                                        <option value="<?=$type?>"
                                                            <?=(isset($service->type) && $service->type == $type)? 'selected': ''?>>
                                                            <?=$service_type?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
