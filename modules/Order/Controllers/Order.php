...
class Order extends BaseController {
    ...
    $this->services_model= new ServiceModel();
	...
	public function index(){
		...
		redirect()->to(base_url("order/add"));
	}
	...
	public function add(){
        ...
        $services = $this->services_model->get_services_list();
		...
		return view('Modules\Order\Views\add\add', $data);
	}
	...
	public function get_services($id = ""){
		...
		if ($check_category) {
			...
			$data = array(
				"module"   		=> "order",
				"services" 		=> $services,
				"custom_rates"  => $user_model->get_custom_rates(),
			 
			);
			
			return view('Modules\Order\Views\add/get_services', $data);
		}		
	}
	...
	public function get_service($id = ""){
		...
		return view('Modules\Order\Views\add/get_service', $data);
	}
	public function ajax_add_order(
		...
		$api_provider_id    = $api_provider_id!=""?$api_provider_id:post("api_provider_id");
		$api_service_id 	= $api_service_id!=""?$api_service_id:post("api_service_id");
		$service_id 		= $service_id!=""?$service_id:post("service_id");
		$cate_id 		    = $cate_id!=""?$cate_id:post("category_id");
		$quantity 		    = $quantity!=""?$quantity:post("quantity");
		$link 		        = $link!=""?$link:post("link");
		$runs               = $runs!=""?$runs:post("runs");
		$interval           = $interval!=""?$interval:post("interval");
		$is_drip_feed       = $is_drip_feed!=""?$is_drip_feed:post("is_drip_feed");
		$is_drip_feed == "on" ? 1 : 0;
		$agree 		        = $agree!=""?$agree:post("agree");
		$agree              = $agree == "on"? 1 : 0;
		$service_type 	    = $service_type!=""?$service_type:post("service_type");
		...
		$check_service  = $this->model->check_record("*", $this->tb_services, $service_id, false, true,false);
	 ...
	 //print_r($check_service);
		/*----------- check refill from service -------------*/
		if($check_service->is_refill)
		{
			$is_refill = "yes";
		}else{
			$is_refill = "no";
		...
		switch ($service_type) {
			...
			case 'custom_comments':
				$comments = strip_tags(post('comments'));
				if ($comments == "") {
					ms(array(
						"status"  => "error",
			...
			case 'custom_comments_package':
				$comments = strip_tags(post('comments_custom_package'));
				if ($comments == "") {
					ms(array(
						"status"  => "error",
						"message" => lang("comments_field_is_required")
	...
	private function save_order($table, $data_orders, $user_balance = "", $total_charge = ""){
		$this->model->common_insert($table, $data_orders);
		...
		if ($this->model->db->affectedRows() > 0) {
			...
			if ($data_orders["service_type"] != "subscriptions") {
				...
				$this->model->common_update($this->tb_users, ["balance" => $new_balance], ["id" => session("uid")]);
			}
	...
	public function ajax_mass_order(){
		...
		if (is_array($mass_order)) {
			...
			if (!empty($orders)) {
				...
				$builder->insert_batch($orders);
				$new_balance = $user->balance - $sum_charge;
				$this->model->common_update($this->tb_users, ["balance" => $new_balance], ["id" => session("uid")]);
			}
		}
		if (!empty($error_details)) {
			return view("Modules\Order\Views\add/mass_order_notification", ["error_details" => $error_details]);
		}else{
			ms(array(
				"status"  => "success",
				"message" => lang("place_order_successfully")
	...
	public function log($order_status = ""){
      //  $this->cachePage(30);
		...
		// $table->setHeading('id','created','link','charge','start_counter','quantity','remains');
			
		// foreach ($order_logs as $sf):
            
		// 	$table->addRow($sf->id,convert_timezone($sf->created, "user"), $sf->link,$sf->charge,$sf->start_counter, $sf->quantity,$sf->remains);
		// endforeach;
	...
	public function log_details($id = ""){
		...
		if (!empty($orders)) {
			...
			return view("Modules\Order\Views\logs/ajax_search", $data);
		}else{
	...
	public function log_update($ids = ""){
		...
		return view('Modules\Order\Views\logs/update', $data);
	}
		...
		if(!empty($check_item)){
			...
			if ($status == "refunded" || $status == "partial" || $status == "canceled") {
				...
				if (!empty($user) && !in_array($check_item->status, array('partial', 'cancelled', 'refunded'))) {
					...
					$this->model->common_update($this->tb_users, ["balance" => $balance], ["id"=> $check_item->uid]);
				}
			...
			$this->model->common_update($this->tb_order, $data, array("ids" => $check_item->ids));
	...
	public function refill_orderx($ids = ""){
		$refill_status	= "yes";
		// $status 		= "processing";
		...
		$this->model->common_update($this->tb_order, $data, array("ids" => $ids));
    ...
    public function refill_order($ids = ""){
		...
		$check_item_refill = $this->model->get("*", $this->tb_order, "ids = '{$ids}'");
		...
		if(!empty($check_item_refill)){
			...
			$check_service  = $this->model->check_record("*", $this->tb_services, $check_item_refill->service_id, false, true);
			$data = array(
				"ids" 	            => ids(),
				"uid" 	            => $this->uid,
				"type" 	            => $check_item_refill->type,
				"cate_id" 	        => $check_service->cate_id,
				"service_id" 	    => $check_service->id,
				"link" 	            => $check_item_refill->link,
				"api_provider_id"  	=> $check_service->api_provider_id,
				"api_service_id"  	=> $check_service->api_service_id,
				"api_order_id"  	=> (!empty($check_service->api_provider_id) && !empty($check_service->api_service_id)) ? -1 : 0,
			//	"api_refill_id"  	=> (!empty($check_service->api_provider_id) && !empty($check_service->api_service_id)) ? -1 : 0,
				"status"			=> $refill_status,
				"service_type"	    => $check_item_refill->service_type,
				"changed" 	        => NOW,
				"created" 	        => NOW,
			);
			$this->model->common_insert($this->tb_refills,$data);
	...
	public function ajax_show_list_custom_mention($ids = ''){
		...
		if (!empty($order)) {
            ...
            if($mentions->exists_list){
				...
				return view('Modules\Order\Views\logs/show_list_custom_mention', $data);
			}
		}
	}
		...
		if ($check_item ) {
			switch ($status) {
				case 'resend_order':
					...
					$data['api_order_id']   = -1;
					$this->model->common_update($this->tb_order, $data, array("ids" => $check_item->ids));
					redirect()->to(cn('order/log/error'));
					break;
